// current date (MM/DD/YY) and time setup
...
// error check
if (!ctx) {
    document.querySelector('.canvas-section').innerHTML = '<p style="color:#7F715D; font-size:0.9rem;">your browser does not support the canvas element</p>';
}
...
// adjust styling for selected color button
function setSelectedColorButton(btn) {
    if (prevColorBtn && prevColorBtn !== btn) {
        prevColorBtn.classList.remove('selected');
    }
...
// archive data and helper functions
...
// canvas event listener logic
resizeCanvas();
window.addEventListener('resize', resizeCanvas);

if (prevColorBtn) {
    setSelectedColorButton(prevColorBtn);
}
